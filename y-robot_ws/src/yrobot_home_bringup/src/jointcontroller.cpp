/*************************************************************************
> File Name: jointcontroller.cpp
> Author: 
> Mail: 
> Created Time: 2018年10月02日 星期二 15时49分25秒
************************************************************************/
#include <ros/ros.h>
...
protected:
    ...
    geometry_msgs::Vector3 joint_motor_msg;
...
public:
        ...
        action_name_(name)
    {
        ...
        fore_arm_joint:4
        tool_joint:5 -->*/
        nh_.param("shouder_arm_joint", MotoName_Id["shouder_arm_joint"], 5);
        nh_.param("upper_arm_joint", MotoName_Id["upper_arm_joint"], 4);
        nh_.param("middle_arm_joint", MotoName_Id["middle_arm_joint"], 3);
        nh_.param("fore_arm_joint", MotoName_Id["fore_arm_joint"], 2);
        nh_.param("tool_joint", MotoName_Id["tool_joint"], 1);

        joint_pub = nh_.advertise<sensor_msgs::JointState>("/move_group/fake_controller_joint_states", 1);

        joint_motor_pub = nh_.advertise<geometry_msgs::Vector3>("/arm_motors", 5000);
...
void publishMotorState(int ids[],std::vector<double> joint_posi){
    ...
    for ( it = joint_posi.begin(); it != joint_posi.end(); it++,i++){
        joint_motor_msg.x=ids[i];
        joint_motor_msg.y=(*it) ;
...
int main(int argc, char** argv)
{
    ...
    RobotTrajectoryFollower RobotTrajectoryFollower("/yrobot_arm_controller/follow_joint_trajectory");
